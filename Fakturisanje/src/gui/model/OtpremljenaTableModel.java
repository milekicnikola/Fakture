...
public class OtpremljenaTableModel extends StandardTableModel {
	 ...
	 * " where otpremljena_roba.sifra_robe = ? and otpremljena_roba.sifra_porudzbine = ? and otpremljena_roba.datum_isporuke = ? and otpremljena_roba.sifra_fakture = ? and otpremljena_roba.status != 'narucena'"
	 * );
	 * 
	 * String f = (String) getValueAt(index, 0); String r = (String)
	 * getValueAt(index, 1); String p = (String) getValueAt(index, 3); String d =
	 * (String) getValueAt(index, 4);
	 * 
	 * selectStmt.setString(1, r); selectStmt.setString(2, p);
	 * selectStmt.setString(3, d); selectStmt.setString(4, f);
	 * 
	 * ResultSet rset = selectStmt.executeQuery();
	 * 
	 * String sifra_robe = "", naziv_robe = "", sifra_porudzbine = "", datum = "",
	 * faktura = "", naruceno = "", komada = "", opis = "", status = ""; Boolean
	 * postoji = false; String errorMsg = ""; while (rset.next()) { sifra_robe =
	 * rset.getString("sifraRobe").trim(); naziv_robe =
	 * rset.getString("NAZIV_ROBE").trim(); sifra_porudzbine =
	 * rset.getString("sifraPorudzbine").trim(); datum =
	 * rset.getString("datumIsporuke"); faktura =
	 * rset.getString("sifraFakture").trim(); naruceno =
	 * rset.getString("KOMADA_NARUCENO"); komada =
	 * rset.getString("KOMADA_FAKTURISANO"); opis = rset.getString("OPIS"); status =
	 * rset.getString("STATUS"); postoji = true; } if (!postoji) { removeRow(index);
	 * fireTableDataChanged(); errorMsg = ERROR_RECORD_WAS_DELETED; } else if
	 * ((SortUtils.getLatCyrCollator().compare(faktura, (String) getValueAt(index,
	 * 0)) != 0) || (SortUtils.getLatCyrCollator().compare(sifra_robe, ((String)
	 * getValueAt(index, 1)).trim()) != 0) ||
	 * (SortUtils.getLatCyrCollator().compare(naziv_robe, (String) getValueAt(index,
	 * 2)) != 0) || (SortUtils.getLatCyrCollator().compare(sifra_porudzbine,
	 * (String) getValueAt(index, 3)) != 0) ||
	 * (SortUtils.getLatCyrCollator().compare(datum, (String) getValueAt(index, 4))
	 * != 0) || (SortUtils.getLatCyrCollator().compare(naruceno, (String)
	 * getValueAt(index, 5)) != 0) || (SortUtils.getLatCyrCollator().compare(komada,
	 * (String) getValueAt(index, 6)) != 0) ||
	 * (SortUtils.getLatCyrCollator().compare(opis, (String) getValueAt(index, 7))
	 * != 0) || (SortUtils.getLatCyrCollator().compare(status, (String)
	 * getValueAt(index, 8)) != 0) ||
	 * (SortUtils.getLatCyrCollator().compare(otpremnica, (String) getValueAt(index,
	 * 9)) != 0)) {
	 * 
	 * setValueAt(faktura, index, 0); setValueAt(sifra_robe, index, 1);
	 * setValueAt(naziv_robe, index, 2); setValueAt(sifra_porudzbine, index, 3);
	 * setValueAt(datum, index, 4); setValueAt(naruceno, index, 5);
	 * setValueAt(komada, index, 6); setValueAt(opis, index, 7); setValueAt(status,
	 * index, 8); fireTableDataChanged(); errorMsg = ERROR_RECORD_WAS_CHANGED; }
...
/*	@Override
	public void search(String[] params) throws SQLException {
		whereStmt = " WHERE otpremljena_roba.sifra_robe LIKE '%" + params[0] + "%' AND "
				+ "otpremljena_roba.naziv_robe LIKE '%" + params[1] + "%' AND "
				+ "otpremljena_roba.sifra_porudzbine LIKE '%" + params[1] + "%' AND "
				+ "narucena_roba.komada_naruceno LIKE '%" + params[1] + "%' AND "
				+ "otpremnica.sifra_otpremnice LIKE '%" + params[1] + "%' AND "
				+ "fakturisana_roba.datum_isporuke LIKE '%" + params[2] + "%' AND "
				+ "fakturisana_roba.sifra_fakture LIKE '%" + params[3] + "%' AND " + "komada_fakturisano LIKE '%"
				+ params[4] + "%' AND " + "opis LIKE '%" + params[5] + "%' AND " + "status LIKE '%" + params[6] + "%'";
	...
	public void fillData(String sql) throws SQLException {
		...
		while (rset.next()) {
			...
			addRow(new String[] { faktura, sifra_robe, naziv_robe, sifra_porudzbine, datum, naruceno, komada, opis,
					status, otpremnica });
		}
		rset.close();
		stmt.close();
	 ...
	 * stmt.executeUpdate(); stmt.close(); DBConnection.getConnection().commit(); if
	 * (rowsAffected > 0) { // i brisanje iz TableModel-a removeRow(index);
	 * fireTableDataChanged(); } }
	 */
	 ...
	 * stmt.setString(3, params[4]); stmt.setString(4, params[0]); stmt.setString(5,
	 * params[5]); stmt.setString(6, params[6]); stmt.setString(7, params[7]);
	 * 
	 * int rowsAffected = stmt.executeUpdate(); stmt.close(); // Unos sloga u bazu
	 * DBConnection.getConnection().commit(); if (rowsAffected > 0) { // i unos u
	 * TableModel retVal = sortedInsert(params); fireTableRowsInserted(retVal,
	 * retVal);
	 * 
	 * } return retVal; }
	 */
	 ...
	 * checkRow(index);
	 * 
	 * String faktura = (String) getValueAt(index, 0); String sifra_robe = (String)
	 * getValueAt(index, 1); String sifra_porudzbine = (String) getValueAt(index,
	 * 3); String datum = (String) getValueAt(index, 4);
	 * 
	 * PreparedStatement stmt = DBConnection .getConnection() .prepareStatement(
	 ...
	 * );
	 * 
	 * stmt.setString(1, params[0]); stmt.setString(2, params[1]); stmt.setString(3,
	 * sifra_robe); stmt.setString(4, sifra_porudzbine); stmt.setString(5, datum);
	 * stmt.setString(6, faktura); stmt.executeUpdate(); stmt.close();
