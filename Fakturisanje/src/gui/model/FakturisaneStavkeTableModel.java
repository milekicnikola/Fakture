...
public class FakturisaneStavkeTableModel extends StandardTableModel {
	...
	public FakturisaneStavkeTableModel(Object[] colName, int rowCount) {
		...
		orderBy = " ORDER BY fakturisana_roba.sifra_robe";
	}
	 ...
	 * rset.getString("KOMADA_POSLATO"); ostalo = rset.getString("KOMADA_OSTALO");
	 * ko = rset.getString("KO_RADI"); korisnik = rset.getString("korisnickoIme");
	 * postoji = true; } if (!postoji) { removeRow(index); fireTableDataChanged();
	 * errorMsg = ERROR_RECORD_WAS_DELETED; } else if
	 * ((SortUtils.getLatCyrCollator().compare(sifra_robe, ((String)
	 * getValueAt(index, 0)).trim()) != 0) ||
	 * (SortUtils.getLatCyrCollator().compare(nazivR, (String) getValueAt(index, 1))
	 * != 0) || (SortUtils.getLatCyrCollator().compare(interni, (String)
	 * getValueAt(index, 2)) != 0) ||
	 * (SortUtils.getLatCyrCollator().compare(sifra_porudzbine, (String)
	 * getValueAt(index, 3)) != 0) || (SortUtils.getLatCyrCollator().compare(datum,
	 * (String) getValueAt(index, 4)) != 0) ||
	 * (SortUtils.getLatCyrCollator().compare(korisnik, (String) getValueAt(index,
	 * 5)) != 0) || (SortUtils.getLatCyrCollator().compare(naruceno, (String)
	 * getValueAt(index, 6)) != 0) ||
	 * (SortUtils.getLatCyrCollator().compare(poslato, (String) getValueAt(index,
	 * 7)) != 0) || (SortUtils.getLatCyrCollator().compare(ostalo, (String)
	 * getValueAt(index, 8)) != 0) || (SortUtils.getLatCyrCollator().compare(ko,
	 ...
	 * fireTableDataChanged(); errorMsg = ERROR_RECORD_WAS_CHANGED; } rset.close();
	 * selectStmt.close(); DBConnection.getConnection().setTransactionIsolation(
	...
	public void search(String[] params) throws SQLException {
		whereStmt = " WHERE fakturisana_roba.sifra_robe LIKE '%" + params[0] + "%' AND "
				+ "fakturisana_roba.sifra_porudzbine LIKE '%" + params[1] + "%' AND "
				+ "fakturisana_roba.sifra_fakture LIKE '%" + params[2] + "%' AND "
				+ "fakturisana_roba.datum_isporuke LIKE '%" + params[3] + "%' AND "
				+ "narucena_roba.korisnicko_ime LIKE '%" + params[4]
	...
	public void fillData(String sql) throws SQLException {
		...
		while (rset.next()) {
			...
			addRow(new String[] { sifra_robe, nazivR, interni, sifra_porudzbine,
					faktura, datum, naruceno, fakturisano, korisnik });
		}
		rset.close();
		stmt.close();
	...
	public void deleteRow(int index) throws SQLException {
		 ...
		 * ); String sifraP = (String) getValueAt(index, 0); String sifraR = (String)
		 * getValueAt(index, 1); String datum = (String) getValueAt(index, 3);
		 * 
		 * stmt.setString(1, sifraP); stmt.setString(2, sifraR); stmt.setString(3,
		 * datum); // Brisanje iz baze int rowsAffected = stmt.executeUpdate();
		 * stmt.close(); DBConnection.getConnection().commit(); if (rowsAffected > 0) {
		 * // i brisanje iz TableModel-a removeRow(index); fireTableDataChanged(); }
		 */
	}
	 ...
	 * stmt.setString(3, params[4]); stmt.setString(4, params[5]); stmt.setString(5,
	 * params[6]); stmt.setString(6, params[3]); stmt.setString(7, params[7]);
	 * 
	...
	public void updateRow(int index, String[] params) throws SQLException {
		 ...
		 * checkRow(index);
		 * 
		 * String sifra_porudzbine = (String) getValueAt(index, 0); String sifra_robe =
		 * (String) getValueAt(index, 1); String datum = (String) getValueAt(index, 3);
		 * 
		 * PreparedStatement stmt = DBConnection .getConnection() .prepareStatement(
		 ...
		 * stmt.executeUpdate(); stmt.close(); DBConnection.getConnection().commit();
		 * setValueAt(params[0], index, 7); fireTableDataChanged();
		 */
	}
